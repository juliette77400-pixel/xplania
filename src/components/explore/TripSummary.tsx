...
const TripSummary = ({ tripId }: Props) => {
  ...
  const generate = async () => {
    ...
    try {
      ...
      toast.error(e?.message || "Échec résumé");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="rounded-2xl border border-border bg-card/40 backdrop-blur-sm p-5 space-y-4">
      <div className="flex items-center justify-between gap-2">
        <h3 className="font-bold flex items-center gap-2"><BookOpen className="w-4 h-4 text-primary" /> Résumé du voyage</h3>
      ...
      {!data && !loading && (
        <p className="text-sm text-muted-foreground">Génère un récit personnalisé de ton voyage à partir de tes lieux visités, badges et souvenirs.</p>
      )}
      ...
      {data && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {[
              { label: "Lieux visités", value: `${data.stats.visited}/${data.stats.total}` },
              { label: "Score", value: data.stats.points },
              { label: "Badges", value: data.stats.badges },
              { label: "Souvenirs", value: data.stats.media },
            ].map((s) => (
          ...
          {data.stats.topTypes.length > 0 && (
            <div className="flex gap-2 flex-wrap">
              {data.stats.topTypes.map(([t, c]) => (
                <span key={t} className="px-2 py-1 rounded-full bg-primary/10 border border-primary/20 text-xs">{t} × {c}</span>
              ))}
            </div>
          )}
